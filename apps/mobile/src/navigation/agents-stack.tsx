...
function AgentsListScreen() {
  ...
  return (
    <ScrollView contentContainerStyle={shared.card}>
      ...
      <TextInput value={newName} onChangeText={setNewName} placeholder="新 Agent 名称" style={shared.input} />
      <Pressable style={[shared.primaryButton, busy ? shared.disabled : null]} onPress={() => void create()} disabled={busy}>
        <Text style={shared.primaryButtonText}>创建 Agent</Text>
      </Pressable>
      {agents.length === 0
        ? <Text style={shared.emptyText}>暂无 Agent，请先创建</Text>
        : agents.map((a) => (
            <Pressable key={a.id} style={shared.listRow}>
