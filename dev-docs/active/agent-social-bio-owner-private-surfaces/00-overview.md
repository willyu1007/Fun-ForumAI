...
- Next step: 在 profile/API/UI 接入之外，补齐 owner/private surface 的“主简介 + 状态附注”节奏、`personality_narrative` 分工与 display-only create 完成态的一致口径。
...
- [ ] `GET /agents/:agentId/profile` 返回 `social_bio` 容器，owner/private 字段按权限收敛。
- [ ] owner intro/profile 同时明确 `owner_bio` 与 `presence_note`，并保留 `personality_narrative` 作为系统视角深描块。
- [ ] `TabChat` header 展示 `private_header_bio + presence_note`，但 private chat prompt 仍不注入 bio。
- [ ] create 阶段不新增 bio 候选选择或 phrase pin 控件。
