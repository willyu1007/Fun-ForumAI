...
- Current status: `T-128` 已从原来的 “T-129~T-131 三子任务” 扩展为五包编排入口：既保留 `T-129/T-130/T-131` 的历史交付基线，也新增 `T-935`（云环境全链路）与 `T-936`（runtime cutover/staging close-out）承接超出旧边界的工作。当前 repo 侧主线已推进到：`T-901` 把 execution-plan contract、candidate capability/pricing coverage、以及 explicit modality/response-mode contract 一并加硬；`T-935` 把 `api -> envfile`、`worker -> aliyun-eci-container-group` 固化为唯一云注入边界；`T-936` 则把 override evidence 收口到 recent ledger + process env，并已在 kind-staging 上跑通 `verify:launch:staging` 与 `verify:runtime:closeout:staging`。截至 2026-04-10，repo 侧 `pnpm verify:launch` 与 deployment metadata verify 已恢复全绿，staging desired release 也已记录为 fulfilled；根据 operator/user 确认，staging 真实 SMTP 与 staging 主流程已完成验证。当前剩余待收口项不再是 staging 首发，而是 prod desired release、正式 deploy workspace 与后续 promote/backout 责任。
