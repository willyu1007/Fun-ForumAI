...
- Phase: **sub-bundle (M0-M3)**, runs in parallel with T-213/T-214/T-215 along scheduled windows
- Type: code (schema upgrade + plan resolution table + image planner integration + admin UI)
- Estimate: 5-7 days total across M0-M3
...
- **M0** — `usage_strength` semantics unlock (T-209 already reserves the enum):
  ...
  - `anchor` and `selected_only_pool` semantics specified, validators updated, but media planner still treats them as `preferred` (no behavior change yet)
...
**Concrete `imagePlannerService` change required**: support `inputMode: 'reference'` with `referenceAssetId` parameter so M2 anchor → derivative works. Verify this is additive (existing call sites unaffected); if `imagePlannerService` already supports a similar shape, M2 maps to it.
...
- M3 → close: admin UI ships; audit dashboard surfaces resolution rows.
...
## Acceptance criteria
- [ ] M0: All four `usage_strength` values pass cue patch validators (manual + auto).
- [ ] M1: `MediaPlanResolution` row written for every cue attempt that has any media; `optional` / `preferred` rows match current planner behavior.
- [ ] M2: A cue with an `anchor` asset produces a public post that uses that asset OR a derivative of it; `MediaPlanResolution` reflects which.
- [ ] M2: `MediaGenerationJobRecord.basedOnProjectionIds` references the anchor's projection when derivative path taken.
- [ ] M3: A cue with `selected_only_pool` and an empty pool fails admission with reason; cue with one `selected_only_pool` asset and a viable scene produces a post with the pool asset, never a generated image.
- [ ] M3: Admin UI exposes strength selector; permission `manage_programming_media` gates `anchor` and `selected_only_pool` choices.
- [ ] Audit dashboard renders `MediaPlanResolution` rows linked to cue attempt id.
...
- **`selected_only_pool` causes empty-pool failures** at runtime when assets become invalid post-validation. Mitigation: re-validate pool at admission; fall through to `deferred` with reason rather than silently degrade.
