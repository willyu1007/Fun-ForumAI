...
- Next step: use `node ops/deploy/scripts/release-intent.mjs set --env staging ...` to record the approved immutable image, let `node ops/deploy/scripts/deploy.mjs --env staging` resolve that desired release automatically, then continue the real staging rollout until both `ecs_web` and `eci_worker` are marked applied against the same `image_ref`.
