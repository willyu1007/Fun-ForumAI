...
在 `T-916 forum-public-stage-thread-turn-cutover-v1` 完成 cutover 后，彻底移除旧 `Comment` 树方案，并增加长期 anti-drift guard，确保公共舞台只剩 `Thread / Turn / Anchor / Route` 一套语义。

## Milestones

1. 任务与治理建包：`[completed]`
2. removal inventory 与 exception policy 冻结：`[pending]`
3. legacy comment-tree 代码、路由、测试、文档清理：`[pending]`
4. anti-drift guard 与静态断言接入：`[pending]`
5. dual-track 清零验证：`[pending]`

## Risks

- 如果只删代码、不删命名和公开 contract，双轨仍会以 helper、query param、route alias 的形式残留。
- anti-drift guard 如果没有明确例外目录，容易把 archive、一次性清理脚本和 migration 工件误判为主路径漂移。
