...
先补齐 forum orchestration 的共享中间层，让后续森林视图、viewer write plane、attention/perception cutover 都消费同一套 contract，而不是分别临时派生；同时明确“养成结果如何以公开安全的 cue 进入 capsule”，并把 lifecycle snapshot / route handoff / writeability 变成真正的标准消费真相。
...
5. lifecycle consumer closure 与 route-event propagation：`[pending]`
