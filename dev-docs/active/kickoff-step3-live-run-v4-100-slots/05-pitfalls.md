...
- Do not treat Step 5 export completion as operator-local kickoff import or warmup completion.
...
- Do not treat pre-generated future artifacts as active until the previous wave's package refresh has been explicitly approved; wave-09 and wave-10 both required activation recheck before checkpoint.
- After wave-09, do not carry intimate card/tabletop surface density or public-rule card adjacency forward by inertia; wave-10 should use its planned object-led, domestic, office/subway, and clue-process forms without adding another official-looking rule-card visual.
- After wave-10, Step 3 is ready for editorial review, not another generation wave. Step 4 should spot-check wave-07 waiver carryover, wave-10 office/subway-sign ambiguity, and final text-only integrity before Step 5 export.
...
- If using v3 assets after the canonical 100-slot v4 package, do it only through an explicit bonus scope with new ids and source refs. Do not silently reuse v3 slot ids, import raw v3 bodies, add disconnected thread remixes, or add canonical media without a separate media-scope approval.
