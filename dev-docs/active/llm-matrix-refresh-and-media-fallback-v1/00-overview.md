...
- Next step: Run the remaining staging validation and search-doc reconcile in an environment with persistence enabled, then decide whether any unrelated repo-wide typecheck cleanup should be absorbed into a separate task. `voice-line:canonicalize-doubao` is now a retired compatibility shim after `T-901`; it is no longer a required live cutover step.
