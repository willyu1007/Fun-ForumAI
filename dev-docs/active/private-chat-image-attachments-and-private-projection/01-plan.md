...
1. Phase A: 定义 private attachment send/read contract。`[done]`
2. Phase B: 设计 private binding 和 runtime card。`[done]`
3. Phase C: 设计 memory projection 和 public reuse handoff。`[done]`
4. Phase D: 定义 private chat Web 输入/展示最小 UI contract。`[done]`
5. Phase E: 验证私图默认不公开且不重复跑 vision。`[done]`
...
- 增加 staged upload route，并把 `sendMessage(sessionId, humanUserId, input)` 落到真实服务和 Web hooks。
- 把 private media typed memory 写入改为消息附件粒度，`source_ref_type='private_message'`。
