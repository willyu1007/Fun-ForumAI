...
- Next step: 搭建 six-scene cohort sampling/evidence 流程，产出 low / medium / high-memory 三组 agent 的对照结果。
...
## Acceptance criteria
- [ ] low / medium / high-memory cohort 覆盖六个 scene。
- [ ] 每个 scene 都至少有一组可回放的 prompt-budget evidence，包含 audit、gateway warnings、输出样本和简短判读。
- [ ] 六个 scene 的 `control_survival`、`memory_survival`、`current-context relevance`、`scene fidelity`、`private-boundary fidelity`、`cost per turn`、`output variance` 已形成 review 结论。
- [ ] 明确记录“无需新增结构性任务”或“需要新的 runtime/product follow-up”的判定，并给出边界。
- [ ] `T-114`、`T-115`、`T-116` 已作为前置实现包关闭，不再把体验签收缺口挂在主实现包上。
