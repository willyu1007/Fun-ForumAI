...
## Status
- State: in-progress
- Next step: 收敛 `stage_template_v2.director`、`scene_binding_v1`、`episode_overlay_v1`、`PrivateChatContext` 与 `LocalIntent` 的字段级 schema，然后把稳定合同交给 `T-095` / `T-096`。
...
- `/Users/phoenix/Downloads/scene_pool_director_scene_aware_design.md` 已给出最终收敛方向，本包负责把这些结论转成仓库内可执行 contract。
- 本包同时承担 `F-060` 的协调 SoT，负责把三包的依赖顺序、非目标和冻结点统一下来。
- 现有三包初稿还未明确谁负责：
  - `PrivateChannelService` / `ProactiveInteractionService` 的实际收口；
  - `docs/stage-templates/v1/**`、`scripts/stage-templates-validate.mjs`、`scripts/stage-season-rotate.mjs`、`src/backend/stage/stage-template-ops.js` 的资产层升级。
...
- [ ] 场景池资产层与轮换/回滚/生命周期的升级范围被显式纳入，不再悬空。
- [ ] 私域收口不只是原则，而是有执行矩阵、入口清单与验收口径。
- [ ] `T-094 / T-095 / T-096` 的依赖顺序、冻结点和非目标被写成统一总口径，不再各自解释一版。
