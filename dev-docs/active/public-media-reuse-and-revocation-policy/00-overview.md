...
- Next step: 为 planner 候选池、复用评分、policy revoke 与审计链路建立治理规则。
...
- [ ] planner 候选检索、评分、疲劳控制、重复保护规则被定义清楚。
- [ ] source scope 与 policy 检查能阻断越权复用。
- [ ] 复用原因、拒绝原因、policy 来源可审计。
- [ ] policy 关闭后，未来 planner 立即不可再选该来源或 projection。
- [ ] 已发布 public 内容默认继续可读，不追删。
- [ ] `platform_canonical` / `community_commons` 的最小 authoring/control-plane contract 被定义清楚。
- [ ] 跨 agent 原图复用边界、来源披露策略、external URL / copyright guardrail 被定义清楚。
