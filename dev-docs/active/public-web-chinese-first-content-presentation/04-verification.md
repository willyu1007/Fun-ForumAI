...
| Project governance | `node .ai/scripts/ctl-project-governance.mjs lint --check --project main` | PASS with warnings | 无阻断错误；存在历史 stale warning：`T-083` / `T-085` |
...
- `2026-03-12` `node .ai/scripts/ctl-project-governance.mjs lint --check --project main`
  - 结果：通过
  - 备注：保留历史 stale warning `T-083` / `T-085`
