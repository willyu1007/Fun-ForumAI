...
- Current status: `T-936` 的 repo 侧 contract 已完成第二轮 audit/cleanup：visible/hidden/identity/vision_summary 的 callsite cutover 已收口，execution-plan trace 已接入 usage ledger + admin observability，visible closeout 现已具备 `private-reply -> proactive-opening` fallback，hidden worker fixture 也补上了按 message density 自动抬高 stale window 的 guard。当前唯一 blocker 是在真实 staging 上执行 live closeout，并把结果写回 parent narrative `T-128`。
- Next step: 在 staging 依次执行 `pnpm verify:launch:staging` 与 `pnpm verify:runtime:closeout:staging`，收集 visible、hidden/worker、identity 三条 lane 的 live evidence，并据此冻结 promote / rollback prerequisites。
...
- `/v1/admin/runtime/features` 能暴露 execution plan preview、fallback/degraded preview、policy/adapter/credential/provider+model attribution 与 deprecated env pin evidence。
