...
Result on this implementation pass: **12/12 assertions passed**.
...
[ok] Missing community commons list → 404 — status=404 code=NOT_FOUND
[ok] Platform upload missing file → 400 — status=400
[summary] 12/12 assertions passed
```
...
- Missing community list returns 404 instead of treating an arbitrary id as a valid `community_commons:<id>` pool.
...
### Post-merge Verification on `main` (2026-04-27)

Run after resolving merge conflicts between `main` (including archived T-301 runtime operation records) and `claude/busy-bardeen-8e7cf9`:

- `node .ai/scripts/ctl-project-governance.mjs sync --apply --project main` → passed; project hub regenerated with T-301 archived and T-302 in-progress.
- `pnpm exec vitest run src/backend/routes/__tests__/admin-media-import-routes.test.ts src/backend/media/__tests__/admin-media-import-service.test.ts src/frontend/features/admin/components/__tests__/MediaImportPanel.test.tsx src/frontend/features/forum/pages/__tests__/CommunitySettingsPage.test.tsx src/backend/media/__tests__/media-reuse-governance-service.test.ts src/backend/routes/__tests__/admin-media-api.test.ts src/backend/media/__tests__/media-injection-medium-regression.integration.test.ts src/backend/media/__tests__/media-injection-retrieval.integration.test.ts --maxWorkers=1` → 47/47 passed across 8 files.
- `pnpm typecheck` → passed, including `db:generate`, `ui:build`, and `tsc -b`.
- `pnpm exec eslint src/backend/media/admin-media-import-service.ts src/backend/media/__tests__/admin-media-import-service.test.ts src/backend/routes/__tests__/admin-media-import-routes.test.ts src/backend/routes/admin/admin-media-routes.ts src/backend/validation/schemas.ts src/frontend/api/hooks/admin.ts src/frontend/features/admin/components/AdminSidebar.tsx src/frontend/features/admin/components/MediaImportPanel.tsx src/frontend/features/admin/components/__tests__/MediaImportPanel.test.tsx src/frontend/features/admin/pages/AdminPages.tsx src/frontend/features/admin/pages/admin-panel/MediaAssetsTab.tsx src/frontend/features/forum/pages/CommunitySettingsPage.tsx src/frontend/features/forum/pages/__tests__/CommunitySettingsPage.test.tsx src/shared/admin-media-import.ts ops/smoke/t302/run-smoke.ts ops/smoke/t302/serve-ui.ts` → passed.
- `NODE_ENV=development APP_ENV=dev PORT=4103 JWT_SECRET=smoke-jwt-secret-not-for-prod SERVICE_AUTH_SECRET=smoke-service-secret-not-for-prod LOG_LEVEL=silent pnpm exec tsx ops/smoke/t302/run-smoke.ts` → 12/12 assertions passed.
- `git diff --check` → passed.

### Post-review Hardening Verification (2026-04-27)

Run after the T-302 code-quality and semantic-closure review:

- `pnpm exec vitest run src/backend/routes/__tests__/admin-media-import-routes.test.ts src/backend/routes/__tests__/admin-media-api.test.ts src/backend/media/__tests__/admin-media-import-service.test.ts src/backend/media/__tests__/media-reuse-governance-service.test.ts --maxWorkers=1` → 36/36 passed. Covers missing-community 404 on upload, URL import, list, and the older low-level commons registration route; also covers invalid upload form `allow_quote_original`.
- `pnpm exec tsc -p tsconfig.json --noEmit --pretty false` → passed.
- `pnpm exec tsx ops/smoke/t302/run-smoke.ts` → 12/12 assertions passed, including missing-community commons list 404.
- `pnpm exec vitest run src/backend/routes/__tests__/admin-media-import-routes.test.ts src/backend/media/__tests__/admin-media-import-service.test.ts src/frontend/features/admin/components/__tests__/MediaImportPanel.test.tsx src/frontend/features/forum/pages/__tests__/CommunitySettingsPage.test.tsx src/backend/media/__tests__/media-reuse-governance-service.test.ts src/backend/routes/__tests__/admin-media-api.test.ts src/backend/media/__tests__/media-injection-medium-regression.integration.test.ts src/backend/media/__tests__/media-injection-retrieval.integration.test.ts --maxWorkers=1` → 51/51 passed across 8 files.
- `pnpm typecheck` → passed after wiring the new `communityRepo` dependency into the direct dev-tool `createLlmServices` call in `src/backend/dev/media-inject.ts`.
- `pnpm exec eslint src/backend/dev/media-inject.ts src/backend/media/admin-media-import-service.ts src/backend/media/media-reuse-governance-service.ts src/backend/routes/admin/admin-media-routes.ts src/backend/validation/schemas.ts src/backend/container/llm.ts src/backend/container/index.ts src/backend/routes/__tests__/admin-media-import-routes.test.ts src/backend/routes/__tests__/admin-media-api.test.ts src/backend/media/__tests__/admin-media-import-service.test.ts ops/smoke/t302/run-smoke.ts` → passed.
- `git diff --check` → passed.
