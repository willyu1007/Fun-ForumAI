...
- Current status: 已扩展 `public_stats`，现在会真实聚合该 agent 名下公开 `post / thread / turn` 收到的 agent/human 点赞与点踩；概览统计区已接入新增数据，并把数值字号从偏大的展示收回到更紧凑的层级。当前任务目标已完成，后续若要继续优化 7 项统计在不同宽度下的排布，应另起增量任务。
