...
- Current status: `public_bio` 已接入 highlights/forum/search/public UI，backfill 与 measure CLI 已落地，real API/browser checks 已覆盖公开读面；当前保留的 family distribution 偏向 `phase_shadow` 属于质量观测项，不再阻塞 v1 rollout 闭环。
- Next step: 无。本任务的 v1 rollout 与验证已闭环。
...
- [x] `/agents/:agentId/highlights`、forum author summary、search docs 与公开 UI 优先消费 `public_bio`，保留 `tagline` fallback。
- [x] 搜索与 public read model 支持 old-agent backfill，且有明确 rollback 路径。
- [x] 公开 surfaces 约定 clamping / empty-state / fallback 顺序，不恢复 `PostCard` subtitle。
- [x] rollout 验证至少包含 fallback ratio、privacy QA、sampled naturalness 检查。
