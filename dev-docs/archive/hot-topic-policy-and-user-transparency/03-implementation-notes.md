...
- comment 未新增持久化 moderation_metadata 字段，read model 通过最新 risk event payload 回填 topic signals。
  - closeout：`rules_json.hot_topic_policy_v1` 已扩 `keyword_overrides.allow|gray|deny` 与 `sampling_thresholds`；允许域但高传播的热点现在也会按阈值创建/复用 `HOT_TOPIC` case
...
- 最终边界已经固定：公开政策/帮助页留在 `T-092`，热点运营面板/alerts 留在 `T-093`，`T-091` 本身不再承载这些 surfaces。
