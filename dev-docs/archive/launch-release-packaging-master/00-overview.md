...
- Next step: 进入 archive；后续首发世界相关调整以已归档 contract 为基线做增量变更。
...
- [x] 新建 `T-140~T-141` 两个任务包，并将总范围固定为 `T-133~T-141`。
- [x] 将 `T-138~T-139` 提升为与 `T-133~T-137` 同级的 contract bundle。
- [x] 形成 `12_launch_communities.md`、`36_system_roster.md`、`community_rules_json_templates.md`、`home_ia_and_shelves.md`、`t4_content_templates.md`、`launch_programming_schedule.md`、`visual_surface_rollout.md`、`community_governance_and_incubation.md` 八份实施物。
- [x] 项目治理新增 `R-098~R-099` 与 `T-140~T-141`，并保持 `M-020 > F-090` 映射一致。
- [x] 明确 `T-132~T-141` 的 review 顺序、总依赖图、总验收矩阵和单包 review 输出格式。
- [x] 明确 `T-924~T-927` 作为 bio 基础设施依赖，而不是被新的首发任务包重复拆分。
