...
- `T-091` 负责 topic policy、漂移/采样判断、kill switch 与在位透明提示，依赖 `T-089` 的 complaint/case 基础。
- `T-092` 负责公开政策/帮助页与前台固定入口，不承载策略执行逻辑。
- `T-093` 负责热点运营面板、实时告警与帖子/房间热度控制，不重写 `T-091` 的策略判定引擎。
...
- `T-092 public-policy-and-help-center-surfaces` -> `R-053`
- `T-093 hot-topic-ops-dashboard-and-alerting` -> `R-053`
...
4. `R-053` 的公开政策页与运营后台拆分交付，但 requirement 口径保持统一为“热点策略与用户透明”。
