...
## Current status
- 状态：done
- 说明：母包现已完成。`T-088~T-093` 的代码、验证、task bundle 与 project hub 口径已统一，`T-089~T-093` 全部归档，`T-087` 本身也可关闭。
...
- 2026-03-13（T-087 closeout）：
  - 收齐 `T-088` 最后一个真实缺口：聊天室 `message` 目标现在具备 `visibility/state/moderation_metadata` 持久化，`GovernanceAdapter` 不再只做存在校验，`QUARANTINE` 不再透给读模型或 SSE，`GRAY` 作为可见但默认折叠内容呈现。
  - 将 `R-053` 重新拆成三段稳定任务边界：`T-091` 保留热点策略引擎与在位透明提示，新增 `T-092` 负责公开政策/帮助页，新增 `T-093` 负责热点运营面板与告警。
  - `HotTopicPolicyService` / `PolicyGatewayService` 已补 `keyword_overrides.allow|gray|deny`、`sampling_thresholds`、sampled-review case 收口，以及帖子 `NO_RECOMMEND` / 房间 `hot_topic_mode` 控制。
  - 前端已新增 `/help`、`/terms`、`/privacy`、`/help/ai-content`、`/help/hot-topic-rules`、`/help/private-chat-verification`、`/help/report-appeal-delete`，并从 `Layout`、`CommunityFeedPage`、`PostDetailPage`、`PrivateChatPage`、`SafetyCenterPage` 暴露固定入口。
  - admin 已新增热点 dashboard / alerts / post distribution / room control API 与 `AdminPanel` hot-topic tab；UI governance gate 已在 `.ai/.tmp/ui/t087-closeout-20260313c/` 归零通过。
  - 将 `T-089`、`T-090`、`T-091` 的历史漂移文案收回真实边界，并将 `T-087~T-091` 连同新增 `T-092/T-093` 统一归档。
