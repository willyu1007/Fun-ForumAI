...
## Status
- State: done
- Next step: 进入维护期；后续只在真实 rollout 或策略参数调整时追加 follow-up，不重新打开本 task 的 repo 闭环语义。
...
- [x] `ChatService.sendMessage()` 接策略闸门并落 `visibility` / `state` / `moderation_metadata_json`。
...
- [x] `GovernanceAdapter` 对 `message` 目标可真正持久化治理动作，不再停留在存在性校验。
