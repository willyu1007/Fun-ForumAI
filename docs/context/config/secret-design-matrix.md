...
| `provider-and-routing` | `dashscope_api_key`, `dashscope_api_key_secondary`, `llm_api_vision`, `zai_api_key`, `zai_api_key_secondary`, `deepseek_api_key`, `kimi_coding_api_key`, `moonshot_api_key`, `minimax_api_key`, `minimax_api_key_secondary`, `tencent_hunyuan_api_key`, `ark_api_key`, `ark_api_key_secondary` | provider-oriented staging Bitwarden keys such as `llm_api_qwen_1`, `llm_api_qwen_2`, `llm_api_glm_1`, `llm_api_glm_2`, `llm_api_deepseek_1`, `llm_api_kimi_1`, `llm_api_kimi_2`, `llm_api_minimax_1`, `llm_api_minimax_2`, `llm_api_tecent_1`, `llm_api_ark_1`, and `llm_api_ark_2` |
...
- `staging` consumes the current operator-owned provider keys `llm_api_qwen_1/2`, `llm_api_glm_1/2`, `llm_api_deepseek_1`, `llm_api_kimi_1/2`, `llm_api_minimax_1/2`, `llm_api_tecent_1`, and `llm_api_ark_1/2`.
- Kimi uses `llm_api_kimi_1` as the Kimi Coding Plan primary (`KIMI_CODING_API_KEY`) and `llm_api_kimi_2` as the ordinary Moonshot `.cn` fallback (`MOONSHOT_API_KEY`).
- Only providers that actually have a second credential in Bitwarden keep a `*-secondary` credential pool in runtime. `deepseek` and `tencent` currently use a single staging credential and rely on cross-provider fallback instead of duplicate-key retry; Kimi separates Coding Plan primary from ordinary Moonshot fallback as different provider domains, not duplicate-key retry.
