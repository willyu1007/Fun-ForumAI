...
import { loadFrontendBuildProfile, toDockerBuildArgs } from './frontend-build-profile.mjs'
...
function parseArgs(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (a === '--build-profile' && args[i + 1]) {
      result.buildProfile = args[++i]
      continue
    }
    if (a === '--help') {
      result.help = true
...
function printPlan(target, tag, prereqs, buildProfile) {
  ...
  console.log(`Build profile: ${buildProfile ? buildProfile.profile : 'default'}`)

  if (buildProfile) {
    console.log('\nFrontend build profile capabilities:')
    for (const [key, value] of Object.entries(buildProfile.frontend_capabilities)) {
  ...
  for (const t of targets) {
    ...
    if (opts.buildProfile) {
      try {
        ...
        console.error(
          `[error] Unable to load build profile "${opts.buildProfile}": ${error.message}`,
        )
        exitCode = 1
        continue
      }
      ...
      dockerBuildArgs = toDockerBuildArgs(buildProfile).flatMap(([key, value]) => [
        '--build-arg',
        `${key}=${value}`,
      ])
    }
