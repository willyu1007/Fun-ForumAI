...
describe('EventAllocator — full pipeline', () => {
  ...
  it('allocates up to event_base quota (NewPostCreated=5)', async () => {
    const { allocator } = buildAllocator(pool)
    const result = await allocator.allocate(makeEvent())
    expect(result.agents.length).toBeLessThanOrEqual(5)
    expect(result.agents.length).toBeGreaterThan(0)
    expect(result.quota_applied).toBe(5)
  })

  it('assigns sequential priorities starting from 1', async () => {
    const { allocator } = buildAllocator(pool)
    const result = await allocator.allocate(makeEvent())
  ...
  it('returns 0 agents for VoteCast (base quota = 0)', async () => {
    const { allocator } = buildAllocator(pool)
    const result = await allocator.allocate(makeEvent({ event_type: 'VoteCast' }))
  ...
  it('prevents same (event_id, agent_id) double allocation', async () => {
    ...
    const r2 = await allocator.allocate(event2)
  ...
  it('excludes over-budget agents', async () => {
    const agents = [
  ...
  it('excludes agents in cooldown', async () => {
    const agents = [
  ...
  it('excludes the event author from allocation', async () => {
    const agents = [makeAgent('agent-author'), makeAgent('other')]
    const { allocator } = buildAllocator(agents)
    const result = await allocator.allocate(makeEvent({ author_agent_id: 'agent-author' }))
...
describe('EventAllocator — degradation', () => {
  ...
  it('moderate degradation tightens quota by 50%', async () => {
  ...
  it('critical degradation reduces quota drastically', async () => {
    const { allocator, degradation } = buildAllocator(pool)
  ...
  it('recovers to normal when lag drops', async () => {
    const { allocator, degradation } = buildAllocator(pool)
    degradation.reportLag(400)
    const r1 = await allocator.allocate(makeEvent())
    expect(r1.degradation_level).toBe('critical')

    degradation.reportLag(0)
    const r2 = await allocator.allocate(makeEvent())
...
describe('EventAllocator — hot-spot thread cap', () => {
  it('stops allocating when thread quota exhausted', async () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const result = await allocator.allocate(event)
