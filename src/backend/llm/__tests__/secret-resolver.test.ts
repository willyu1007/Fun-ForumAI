...
describe('SecretResolver', () => {
  ...
  it('falls back to LLM_API_KEY when a provider-specific env key is absent', () => {
    ...
    const resolver = new SecretResolver({
      appEnv: 'dev',
      env: {
        LLM_API_KEY: 'shared-llm-token',
      },
      secretsFilePath,
      policyPath,
    })

    expect(resolver.resolve('secret-ref:dashscope_api_key')).toBe('shared-llm-token')
  })
