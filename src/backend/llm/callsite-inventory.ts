...
export const LLM_CALLSITE_INVENTORY: LlmCallsiteInventoryEntry[] = [
  {
    ...
    raw_model_notes: 'Forum-post body generation keeps the base profile family but now binds a dedicated low-latency execution policy to reduce root-event tail latency without changing profile authority.',
    target_gateway_surface: 'LLMGateway.generateVisibleText',
    target_policy_id: 'visible-forum_reply-post-base',
  ...
  },
  {
    ...
    expected_profile_refs: {
      ...
      'glm-deep-v1': 'glm-deep-forum-reply-lite',
    },
  ...
  {
    source_id: 'agent-executor-forum-action-plan',
    source_file: 'src/backend/runtime/agent-executor.ts',
    scene: 'forum_thread',
    dispatch_calls: ['llmGateway.generateVisibleText'],
    evidence_patterns: [
    ...
    intent: 'forum_reply',
    visibility: 'visible',
    prompt_ref: { id: 'agent-plan-forum-actions', version: 1 },
    voice_line_authority: 'Forum action planning stays on forum_reply routing authority, but uses a dedicated lite json-object execution policy separate from body generation.',
    tier_floor: 'lite',
    expected_profile_refs: {
      'qwen-social-v1': 'qwen-social-forum-reply-lite',
      'glm-deep-v1': 'glm-deep-forum-reply-lite',
    },
    raw_model_notes: 'Structured forum planning must not reuse the free-text body-generation policy; keep this dedicated json-object callsite separate.',
    target_gateway_surface: 'LLMGateway.generateVisibleText',
    target_policy_id: 'visible-forum_reply-action-plan-lite',
    policy_binding_mode: 'callsite-execution-policy',
    migration_status: 'migrated',
    local_override_fields: [],
    local_override_notes: null,
    migration_blocker: 'Keep this entry to guard the structured planner callsite and prevent forum runtime from drifting back to free-text action parsing.',
  },
  {
    ...
    raw_model_notes: 'Forum-thread body generation keeps the base profile family but now binds a dedicated low-latency execution policy to reduce child-event tail latency.',
    target_gateway_surface: 'LLMGateway.generateVisibleText',
    target_policy_id: 'visible-forum_reply-thread-base',
    policy_binding_mode: 'callsite-execution-policy',
    migration_status: 'migrated',
    local_override_fields: [],
    local_override_notes: null,
  ...
  {
    source_id: 'forum-scene-director-plan-enrichment',
    source_file: 'src/backend/services/forum-director-plan-enrichment-service.ts',
    scene: 'background_hidden',
    dispatch_calls: ['llmGateway.generateHiddenArtifact'],
    evidence_patterns: [
      'PROMPT_TEMPLATE_REFS.internalForumScenePlan',
      'this.deps.llmGateway.generateHiddenArtifact({',
      "intent: 'director_plan'",
    ],
    intent: 'director_plan',
    visibility: 'hidden',
    prompt_ref: { id: 'internal-forum-scene-plan', version: 1 },
    voice_line_authority: 'Forum root scene enrichment must stay on the hidden director line so public scene authority remains deterministic while the planner only adds bounded enrichment.',
    tier_floor: 'base',
    expected_profile_refs: {
      'qwen-director-v1': 'qwen-director-director-plan-base',
    },
    raw_model_notes: 'Forum root scene enrichment uses the registry-owned hidden director_plan base lane without any callsite execution-policy override.',
    target_gateway_surface: 'LLMGateway.generateHiddenArtifact',
    target_policy_id: 'hidden-director_plan-base',
    policy_binding_mode: 'profile-default',
    migration_status: 'migrated',
    local_override_fields: [],
    local_override_notes: null,
    migration_blocker: 'Keep this entry to prevent forum root scene planning from drifting back into ad-hoc prompt assembly or raw model overrides outside the hidden director lane.',
  },
  {
    // T-214 A-M3 follow-on closer — auto-editor LLM call site.
    source_id: 'cue-auto-editor',
    source_file: 'src/backend/programming/auto-editor/llm-gateway-auto-cue-editor-adapter.ts',
    scene: 'background_hidden',
    dispatch_calls: ['llmGateway.generateHiddenArtifact'],
    evidence_patterns: [
      "intent: 'director_plan'",
      "responseMode: 'json_object'",
      'this.deps.llmGateway.generateHiddenArtifact(request)',
      'promptRef: PROMPT_TEMPLATE_REFS.cueAutoEditor',
      'class LLMGatewayAutoCueEditorAdapter',
    ],
    intent: 'director_plan',
    visibility: 'hidden',
    prompt_ref: { id: 'cue-auto-editor', version: 1 },
    voice_line_authority: 'Auto-editor traffic rides the existing hidden director_plan lane so cue patches share routing authority with the rest of the director-track planning surface. A future split into a dedicated `cue_auto_edit` intent is documented but deferred until the routing artifact gets a programming-side family.',
    tier_floor: 'base',
    expected_profile_refs: {
      'qwen-director-v1': 'qwen-director-director-plan-base',
    },
    raw_model_notes: 'Adapter renders through the registered `cue-auto-editor` v1 prompt template and passes structured JSON input via prompt variables; validator chain still rejects any off-schema response.',
    target_gateway_surface: 'LLMGateway.generateHiddenArtifact',
    target_policy_id: 'hidden-director_plan-base',
    policy_binding_mode: 'profile-default',
    migration_status: 'dual-track',
    local_override_fields: [],
    local_override_notes: 'Adapter forwards retry temperature bias as a system-message conservativeness directive (no localOverrides path until the gateway exposes a temperature-override surface).',
    migration_blocker: 'Splitting auto-editor onto a dedicated `cue_auto_edit` intent requires a new entry in VOICE_LINE_ROUTING_INTENTS plus matching profile mappings in the routing artifact. Tracked separately; current dual-track via director_plan is operational.',
  },
  {
  ...
  {
    source_id: 'agent-biography-chapter-render',
    source_file: 'src/backend/services/biography-writer-service.ts',
    scene: 'background_hidden',
    dispatch_calls: ['llmGateway.generateHiddenArtifact'],
    evidence_patterns: [
    ...
    intent: 'public_observation_digest',
    visibility: 'hidden',
    prompt_ref: { id: 'internal-agent-biography-chapter-render', version: 2 },
    voice_line_authority: 'Biography chapter render binds to a hidden biography-only routing line so chapter prose can adopt a specialized model order without affecting other hidden digest callsites.',
    tier_floor: 'premium',
    expected_profile_refs: {
    ...
    local_override_fields: [],
    local_override_notes: null,
    migration_blocker: 'Keep this entry to prevent biography chapter render from drifting back onto shared director digest routing or prompt-coupled model overrides.',
  },
  {
    source_id: 'agent-biography-later-note-render',
    source_file: 'src/backend/services/biography-writer-service.ts',
    scene: 'background_hidden',
    dispatch_calls: ['llmGateway.generateHiddenArtifact'],
    evidence_patterns: [
    ...
    intent: 'public_observation_digest',
    visibility: 'hidden',
    prompt_ref: { id: 'internal-agent-biography-later-note-render', version: 1 },
    voice_line_authority: 'Biography later notes stay on the biography-only hidden line, but bind the base tier so inline addenda favor stable structured rendering over richer prose.',
    tier_floor: 'base',
    expected_profile_refs: {
    ...
    local_override_fields: [],
    local_override_notes: null,
    migration_blocker: 'Keep this entry to prevent inline later-note rendering from sharing chapter-tier routing or silently falling back to the shared director line.',
  },
  {
