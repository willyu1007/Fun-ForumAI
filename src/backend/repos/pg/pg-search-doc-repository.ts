...
import type { PublicActorType } from '../types/forum.js'
import type {
  AgentSearchDoc,
  CommunitySearchDoc,
  PostSearchDoc,
...
function toFormatCapabilities(value: unknown): AgentSearchDoc['format_capabilities'] {
  return toJsonArray(value)
    .map((item) => normalizeFormatCapabilityId(item))
    .filter((item): item is AgentSearchDoc['format_capabilities'][number] => item !== null)
}

function normalizePublicActorType(value: string | null | undefined): PublicActorType {
...
export class PgSearchDocRepository implements SearchDocRepository {
  ...
  private async queryPostDocs(input: SearchDocQueryInput): Promise<Array<PostSearchDoc & { score: number }>> {
    ...
    return rows.map((row) => ({
      ...
      community_family: normalizeCommunityFamily(row.community_family),
      community_shell_category: normalizeCommunityShellCategory(row.community_shell_category),
      publication_review_profile_id: normalizePublicationReviewProfileId(row.publication_review_profile_id),
      public_participation_mode: normalizePublicParticipationMode(row.public_participation_mode),
      community_lifecycle_state: normalizeCommunityLifecycleState(row.community_lifecycle_state),
      launch_wave: row.launch_wave,
      author_agent_id: row.author_agent_id,
      author_display_name: row.author_display_name,
      author_avatar_url: row.author_avatar_url,
      author_identity_role_id: normalizeIdentityRoleId(row.author_identity_role_id),
      author_identity_visibility_role_id: normalizeIdentityVisibilityRoleId(row.author_identity_visibility_role_id),
      ...
      scene_phase: normalizeScenePhase(row.scene_phase),
      storyline_state: normalizeStorylineState(row.storyline_state),
      aftershow_text: row.aftershow_text,
      highlight_text: row.highlight_text,
      content_kind: normalizeContentKind(row.content_kind),
      format_kind: normalizeFormatKind(row.format_kind),
      editorial_shelf_id: normalizeEditorialShelfId(row.editorial_shelf_id),
      note_template_id: row.note_template_id,
      cover_mode: row.cover_mode,
      surface_kind: normalizeLaunchSurfaceKindId(row.surface_kind),
  ...
  private async queryCommunityDocs(input: SearchDocQueryInput): Promise<Array<CommunitySearchDoc & { score: number }>> {
    ...
    return rows.map((row) => ({
      ...
      publication_review_profile_id: normalizePublicationReviewProfileId(row.publication_review_profile_id),
      public_participation_mode: normalizePublicParticipationMode(row.public_participation_mode),
      audience_signal_ingestion: normalizeAudienceSignalIngestion(row.audience_signal_ingestion),
      agent_human_response_mode: normalizeAgentHumanResponseMode(row.agent_human_response_mode),
      community_lifecycle_state: normalizeCommunityLifecycleState(row.community_lifecycle_state),
      launch_wave: row.launch_wave,
      description: row.description,
      dominant_tags_summary: row.dominant_tags_summary,
      resident_agent_names_text: row.resident_agent_names_text,
      representative_post_title: row.representative_post_title,
      representative_post_snippet: row.representative_post_snippet,
      scene_tags_text: row.scene_tags_text,
      searchable_text: row.searchable_text,
      activity_7d: row.activity_7d,
      activity_30d: row.activity_30d,
      active_member_count: row.active_member_count,
      representative_post_id: row.representative_post_id,
      representative_agent_id: row.representative_agent_id,
      refreshed_at: row.refreshed_at,
      created_at: row.created_at,
      updated_at: row.updated_at,
      score: row.score,
  ...
  private async queryThreadDocs(input: SearchDocQueryInput): Promise<Array<ThreadSearchDoc & { score: number }>> {
    ...
    return rows.map((row) => ({
      ...
      community_lifecycle_state: normalizeCommunityLifecycleState(row.community_lifecycle_state),
      launch_wave: row.launch_wave,
      author_actor_type: normalizePublicActorType(row.author_actor_type),
      author_agent_id: row.author_agent_id,
  ...
  private toPostDoc(row: {
    ...
    return {
      ...
      community_shell_category: normalizeCommunityShellCategory(row.communityShellCategory),
      publication_review_profile_id: normalizePublicationReviewProfileId(row.publicationReviewProfileId),
      public_participation_mode: normalizePublicParticipationMode(row.publicParticipationMode),
      community_lifecycle_state: normalizeCommunityLifecycleState(row.communityLifecycleState),
      ...
      author_identity_role_id: normalizeIdentityRoleId(row.authorIdentityRoleId),
      author_identity_visibility_role_id: normalizeIdentityVisibilityRoleId(row.authorIdentityVisibilityRoleId),
      ...
      scene_phase: normalizeScenePhase(row.scenePhase),
      ...
      surface_kind: normalizeLaunchSurfaceKindId(row.surfaceKind),
  ...
  private toCommunityDoc(row: {
    ...
    return {
      ...
      publication_review_profile_id: normalizePublicationReviewProfileId(row.publicationReviewProfileId),
      public_participation_mode: normalizePublicParticipationMode(row.publicParticipationMode),
      audience_signal_ingestion: normalizeAudienceSignalIngestion(row.audienceSignalIngestion),
      agent_human_response_mode: normalizeAgentHumanResponseMode(row.agentHumanResponseMode),
      community_lifecycle_state: normalizeCommunityLifecycleState(row.communityLifecycleState),
      launch_wave: row.launchWave,
      description: row.description,
  ...
  private toThreadDoc(row: {
    ...
    return {
      ...
      community_lifecycle_state: normalizeCommunityLifecycleState(row.communityLifecycleState),
      launch_wave: row.launchWave,
      author_actor_type: normalizePublicActorType(row.authorActorType),
      author_agent_id: row.authorAgentId,
