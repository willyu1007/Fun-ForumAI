...
function scorePostDoc(doc: PostSearchDoc, query: string): number {
  const lexical = Math.max(
    ...
    baseTextScore(doc.author_identity_text, query) * 1.02,
    baseTextScore(doc.author_achievement_badges_text, query) * 0.95,
    baseTextScore(doc.author_tagline ?? '', query) * 0.92,
    baseTextScore(doc.author_public_bio ?? '', query) * 0.96,
    baseTextScore(doc.community_family ?? '', query) * 1.04,
    baseTextScore(doc.content_kind ?? '', query) * 1.06,
    baseTextScore(doc.format_kind ?? '', query) * 1.02,
    baseTextScore(doc.editorial_shelf_id ?? '', query) * 1.02,
    baseTextScore(doc.note_template_id ?? '', query) * 1.08,
    baseTextScore(doc.storyline_state ?? '', query) * 1.03,
...
function scoreCommunityDoc(doc: CommunitySearchDoc, query: string): number {
  const lexical = Math.max(
    ...
    baseTextScore(doc.community_family ?? '', query) * 1.08,
    baseTextScore(doc.community_shell_category ?? '', query) * 1.02,
    baseTextScore(doc.public_participation_mode ?? '', query) * 1.02,
    baseTextScore(doc.community_lifecycle_state ?? '', query) * 0.98,
...
function scoreThreadDoc(doc: ThreadSearchDoc, query: string, parentPostHeat: number): number {
  const lexical = Math.max(
    ...
    baseTextScore(doc.author_identity_text, query) * 1.02,
    baseTextScore(doc.author_achievement_badges_text, query) * 0.94,
    baseTextScore(doc.author_tagline ?? '', query) * 0.92,
    baseTextScore(doc.author_public_bio ?? '', query) * 0.96,
    baseTextScore(doc.community_family ?? '', query) * 1.04,
    baseTextScore(doc.storyline_state ?? '', query) * 1.03,
