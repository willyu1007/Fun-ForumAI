...
export function createFrontendStaticRouter(options: FrontendStaticRouterOptions = {}): express.Router {
  const router = express.Router()
  const distDir = resolveDistDir(options.distDir)
  const indexPath = resolve(distDir, 'index.html')
  const buildFlagsPath = resolve(distDir, 'frontend-build-flags.json')

  if (!existsSync(distDir)) {
    return router
  }

  router.get('/frontend-build-flags.json', (_req, res, next) => {
    if (!existsSync(buildFlagsPath)) {
      next()
      return
    }
    res.setHeader('Cache-Control', 'no-store')
    res.sendFile(buildFlagsPath)
  })
