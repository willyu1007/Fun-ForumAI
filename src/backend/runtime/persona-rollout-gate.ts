...
export function buildPersonaRolloutPreReview(
  ...
  const warningResults = input.offlineGate.results.filter((result) => result.kind === 'guardrail')
  ...
  const callsiteDeltas: PersonaRolloutCallsiteDeltaV1[] = [
    ...PERSONA_T070_BLOCKING_CALLSITES.map((callsite) => {
      ...
      const status: PersonaRolloutCallsiteDeltaV1['status'] = delta > 0 ? 'pass' : 'fail'
      if (delta <= 0) {
        issues.push({
          code: `callsite-${callsite}-not-advanced`,
          severity: 'blocking',
          message: `${callsite} did not advance during T-070 shadow logging.`,
        })
      }
      return {
...
function deriveRecommendation(
  ...
  const requiredSlicesHealthy = sliceSummaries
    ...
    .every((summary) => summary.available_samples > 0 && summary.completed && summary.status === 'pass')
...
export function renderPersonaRolloutVerdictMarkdown(
  ...
  const lines = [
    ...
    '## Callsite Coverage',
    '',
    '| callsite | blocking | before | after | delta | status |',
    '| --- | --- | ---: | ---: | ---: | --- |',
    ...snapshot.callsite_deltas.map((item) =>
      `| ${item.source_callsite_id} | ${item.blocking ? 'yes' : 'no'} | ${item.before_total} | ${item.after_total} | ${item.delta} | ${item.status} |`,
    ),
    '',
    '## Issues',
    '',
