...
describe('ProactiveInteractionService', () => {
  ...
  it('records db_diagnostic when AgentRun persistence fails', async () => {
    const recorder = vi.fn()
    setRuntimeOperationRecorder(recorder)
    ...
    const service = new ProactiveInteractionService({
      channelRepo: {
        ...
        createMessage: vi.fn(async () => ({
          id: 'msg-1',
          session_id: 'session-db-diagnostic',
          author_type: 'AGENT',
          content: 'opening',
          created_at: new Date(),
        })),
      ...
      agentRunRepo: {
        create: vi.fn(() => {
          throw new Error('postgres://user:secret@localhost/app insert failed')
        }),
      } as never,
      notificationService: { create: vi.fn(async () => ({ id: 'notif-1' })) } as never,
    })
    ...
    expect(recorder).toHaveBeenCalledWith(expect.objectContaining({
      ...
      error_message_redacted: expect.not.stringContaining('secret'),
