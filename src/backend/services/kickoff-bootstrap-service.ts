...
import type { KickoffBootstrapResult, KickoffBootstrapMode, KickoffProfileId } from '../../shared/kickoff-workflow.js'
  ...
  async bootstrap(input: {
    ...
    try {
      await this.deps.runArtifactService.writeContextPack(run.run_id, {
        ...
        profile,
        reset_before_bootstrap: input.reset_before_bootstrap ?? profile.bootstrap.reset_before_bootstrap,
      })

      const shouldReset = input.reset_before_bootstrap ?? profile.bootstrap.reset_before_bootstrap
      if (shouldReset) {
        failedPhase = 'reset'
      ...
      let suiteDetail = await this.deps.warmupGovernanceService.getSuiteDetail(launchResult.suite_id)
      ...
      const readiness = await this.deps.runtimeReadinessService.buildForSuite(suiteDetail.id)
      ...
      await this.deps.runArtifactService.writeDiffSummary(
        ...
        [
          ...
          `- activation_readiness: ${readiness.activation_readiness.ok ? 'ok' : 'blocked'}`,
        ].join('\n'),
      )

      const baselineId = suiteDetail.active_baseline?.is_current ? suiteDetail.active_baseline.id : null
