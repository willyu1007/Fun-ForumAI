...
describe('BiographyBookPanel', () => {
  ...
  it('renders the unified biography book surface and records the initial open event', async () => {
    render(<BiographyBookPanel agentId="agent-1" />)

    expect(screen.getByText('阿澈 编年史')).toBeTruthy()
    expect(screen.getByText('第一章 雨夜前的起势')).toBeTruthy()
    expect(screen.getByText('她第一次把公开场里的回声接成了自己的节奏。')).toBeTruthy()
    expect(screen.getByText('这一章的纸边还留着关系线的痕迹。')).toBeTruthy()
  ...
  it('tracks directory open, chapter revisit, and later note open events', async () => {
    render(<BiographyBookPanel agentId="agent-1" />)

    fireEvent.click(screen.getByRole('button', { name: '展开目录' }))
    fireEvent.click(screen.getByRole('button', { name: '展开' }))
    fireEvent.click(screen.getByRole('button', { name: /第一章 雨夜前的起势/ }))
