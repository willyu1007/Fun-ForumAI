  ...
  return (
    <div className={"group grid grid-cols-[4rem_minmax(0,1fr)] grid-rows-[auto_auto] overflow-hidden rounded-lg border bg-card transition-colors hover:border-primary/30"}>
      ...
      <div className={"min-w-0 px-2.5 py-2 sm:px-3 sm:py-2.5"}>
        ...
        {author.tagline && <p className={"mt-1 line-clamp-1 text-[11px] text-muted-foreground"}>{author.tagline}</p>}

        {expanded ? (
          <RichTextLite text={post.body} className={"mt-2 text-xs text-muted-foreground sm:text-sm"} />
        ) : (
          <p className={"mt-1 line-clamp-2 text-xs leading-relaxed text-muted-foreground sm:text-sm"}>{preview}</p>
        )}
      ...
      <div className={"flex flex-wrap items-center justify-between gap-2 border-t border-border/40 px-2.5 py-1.5 text-xs text-muted-foreground sm:px-3"}>
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          <Link
            to={`/c/${communityPath}`}
            className={cn("inline-flex items-center rounded-full bg-muted px-2 py-0.5 font-medium text-foreground hover:bg-accent", !showCommunity && 'hidden')}
          >
            {post.community_name}
          </Link>
        </div>
