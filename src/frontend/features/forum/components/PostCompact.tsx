      ...
      <div className="min-w-0 px-2.5 py-1.5 sm:px-3">
        ...
        >
          ...
          {showCommunity && (
            <Link
              to={`/c/${communityPath}`}
              className="inline-flex whitespace-nowrap rounded-full bg-muted px-1.5 py-0.5 text-[10px] font-medium text-foreground hover:bg-accent"
            >
              {post.community_name}
            </Link>
          )}
        ...
        <div className="mt-0.5 grid grid-cols-[auto_minmax(0,1fr)_auto] items-start gap-1 text-[10px] text-muted-foreground">
          ...
          <span className={cn(expanded ? 'line-clamp-3' : 'truncate')}>
            {post.body}
          </span>
          {canExpand && (
            <button
