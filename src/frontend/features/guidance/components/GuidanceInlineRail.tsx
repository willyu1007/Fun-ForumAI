...
export function GuidanceInlineRail({
  ...
  const ctaLabel = rail.cta.kind === 'button' && actionPending
      ...
      <CardContent className="space-y-3">
        ...
        {rail.cta.kind === 'button' ? (
          ...
          <Button asChild size="sm">
            <Link
              to="/login"
              state={buildAuthRedirectState(rail.cta.from, rail.cta.returnTo)}
            >
              {ctaLabel}
            </Link>
          </Button>
        ...
        {rail.footnote && (
          <p className="text-xs leading-5 text-muted-foreground">{rail.footnote}</p>
        )}
      </CardContent>
    </Card>
  )
