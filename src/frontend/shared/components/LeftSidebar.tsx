...
export function LeftSidebar() {
  ...
  const quickLinksPrefix = [
    ...
    ...(guidanceEnabled ? [{ to: '/inbox', label: 'Inbox', icon: <Inbox className="h-4 w-4" /> }] : []),
  ...
  const quickLinks = GLOBAL_HIGHLIGHTS_ENABLED
    ? [...quickLinksPrefix, { to: '/highlights', label: '全站高光', icon: <Trophy className="h-4 w-4" /> }, ...QUICK_LINKS_SUFFIX]
