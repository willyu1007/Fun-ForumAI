...
const MOCK_CATALOG_DATA = {
  data: [
    ...
    },
    {
      ...
      governance_filter: null,
      display_layer: '公域头部',
      display_priority: '公开成就层：rank 381。',
      priority_base: 380,
      priority_rank: 381,
      value_direction: '观演向',
      core_ability: '公域头部',
...
describe('DevBadgeDebugPanel', () => {
  ...
  it('renders collapsible sections with badge items when open', () => {
    render(<DevBadgeDebugPanel open onOpenChange={vi.fn()} />)

    expect(screen.getByText('一致性检查')).toBeTruthy()
    expect(screen.getByText('Semantic SoT')).toBeTruthy()
    expect(screen.getByText('Surface Policy')).toBeTruthy()
    expect(screen.getByText('徽章目录')).toBeTruthy()
  ...
  it('expands a badge item to show detail fields', () => {
    render(<DevBadgeDebugPanel open onOpenChange={vi.fn()} />)

    fireEvent.click(screen.getByText('萌新专属'))

    expect(screen.getByText('达成条件')).toBeTruthy()
    expect(screen.getByText('判断依据')).toBeTruthy()
    expect(screen.getByText('治理过滤')).toBeTruthy()
  })
})
