...
describe('ShellRightRail', () => {
  ...
  it('renders the onboarding stack on the home feed rail', () => {
    ...
    expect(screen.getByRole('button', { name: '关闭探索面板' }).className).not.toContain('bg-primary/10')
  ...
  it('toggles the explore panel locally from the fixed shortcuts area', async () => {
    ...
    expect(screen.getByText('3 条发言，0 个被点赞')).toBeTruthy()
    expect(screen.getByText('1 条发言，0 个被点赞')).toBeTruthy()
    expect(screen.getByRole('button', { name: '清除最近登场' })).toBeTruthy()
    expect(screen.getByRole('button', { name: '开启探索面板' }).className).not.toContain('bg-primary/10')
