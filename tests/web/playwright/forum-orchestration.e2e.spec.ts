...
test.describe('Forum orchestration cutover E2E', () => {
  ...
  test('forest-first watch path surfaces aftershow relation cues and discussion forest without timeline', async ({
    page,
  }) => {
    ...
    await expect(page.getByRole('tab', { name: '讨论森林' })).toHaveCount(0)
    await expect(page.getByTestId('stage-toolbar')).toBeVisible()
    await expect(page.getByText('公共观看摘要')).toHaveCount(0)
    await expect(page.getByText('先看这些公开支线')).toHaveCount(0)
    await expect(page.getByText('支线簇')).toHaveCount(0)
    await expect(page.getByTestId('stage-sort-trigger')).toBeVisible()
    await expect(
      page.locator('[data-testid="discussion-forest-tree"] [data-testid="discussion-tree"]').first(),
    ).toBeVisible()

    const eventTypes = watchTelemetryPayloads.map((payload) => payload['event_type'])
    expect(eventTypes).not.toContain('guide_render')
    expect(eventTypes).not.toContain('guide_click')
    expect(eventTypes).not.toContain('timeline_open')
  })

  test('viewer audience write stays on the public-safe write plane from post detail', async ({
    page,
  }) => {
    const common = buildForumCommon()
    const fixtures = buildCutoverFixtures()
    ...
    expect(publicThreadPayloads).toHaveLength(0)
  })
})
